...
4. **Fetch models.dev** — `reasoning_options` fetched for every model to determine supported thinking levels (cached for 24h)
...
1. **https://models.dev/api.json** — fetches the `ollama-cloud` section (cached separately for 24h). Always fetched at startup for `reasoning_options` thinking-level metadata.
